...
<template>
  <section>
    ...
    <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">Daftar Reasons</h2>
    </template>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
          <div class="p-6 text-gray-900">
            <div class="mb-4">
              ...
              <inertia-link :href="route('reasons.create')" class="bg-blue-500 text-white px-4 py-2 rounded">Tambah Testimoni</inertia-link>
            </div>
              ...
              <tbody>
                ...
                <tr v-for="reason in reasons" :key="reason.id">
                  ...
                  <td class="px-4 py-2 border">
                    <img v-if="reason.icon" :src="`/storage/icons/${reason.icon}`" alt="Icon-Reason" class="media-thumbnail">
                    <p v-else class="w-10 h-10 rounded-full bg-gray-300"></p> <!-- Placeholder jika tidak ada foto -->
                  </td>

                  <!-- Judul -->
                  <td class="px-4 py-2 border">{{ reason.title }}</td>

                  <!-- Deskripsi -->
                  <td class="px-4 py-2 border">{{ truncateDescription(reason.description) }}</td>

                  <!-- Aksi -->
                  <td class="px-4 py-2 border">
                    <inertia-link :href="route('reasons.edit', reason.id)" class="bg-yellow-500 text-white px-4 py-2 rounded">Edit</inertia-link>
                    <inertia-link :href="route('reasons.show', reason.id)" class="bg-green-500 text-white px-4 py-2 rounded">Show</inertia-link>
                    <button @click="deleteReason(reason.id)" class="bg-red-500 text-white px-4 py-2 rounded">Hapus</button>
                  </td>
                </tr>
              </tbody>
            </table>
