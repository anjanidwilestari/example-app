<template>
  ...
  import { Inertia } from '@inertiajs/inertia'; // Assuming you're using Inertia for navigation
  import AuthenticatedLayout from '@/Layouts/AuthenticatedLayout.vue';
  
  // Mendapatkan data buku dari props yang diterima
  const { books, currentBook: initialCurrentBook } = usePage().props;
  
  // Menyimpan ID buku yang dipilih
  const selectedBookId = ref(initialCurrentBook ? initialCurrentBook.id : null);
  
  // Menyimpan buku yang akan ditampilkan (local state)
  const currentBook = ref(initialCurrentBook || null); // Renamed local variable to avoid conflict
  
  // Fungsi untuk mengupdate buku yang dipilih
  const updateBook = () => {
    // Pastikan books dan selectedBookId sudah ada
    if (!books || !selectedBookId.value) return;
  
    const selectedBook = books.find(book => book.id === selectedBookId.value);
    
    // Cek apakah buku ditemukan sebelum mengupdate currentBook
    if (selectedBook) {
